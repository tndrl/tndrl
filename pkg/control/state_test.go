...
func TestNewState(t *testing.T) {
	s := NewState("spiffe://tndrl/node/test")

	if s.GetState() != tndrlv1.NodeState_NODE_STATE_STARTING {
		t.Errorf("expected STARTING state, got %v", s.GetState())
	}
	if s.GetIdentity() != "spiffe://tndrl/node/test" {
		t.Errorf("expected identity 'spiffe://tndrl/node/test', got %v", s.GetIdentity())
	}
...
func TestStateTransitions(t *testing.T) {
	...
	// STARTING -> READY
	s.SetReady()
	if s.GetState() != tndrlv1.NodeState_NODE_STATE_READY {
		t.Errorf("expected READY state, got %v", s.GetState())
	}

	// READY -> DRAINING
	s.SetDraining()
	if s.GetState() != tndrlv1.NodeState_NODE_STATE_DRAINING {
		t.Errorf("expected DRAINING state, got %v", s.GetState())
	}

	// DRAINING -> STOPPED
	s.SetStopped()
	if s.GetState() != tndrlv1.NodeState_NODE_STATE_STOPPED {
...
func TestTaskTracking(t *testing.T) {
	...
	if s.GetState() != tndrlv1.NodeState_NODE_STATE_BUSY {
